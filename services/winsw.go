...
func (ws *WinSWService) Uninstall() error {
	// Stop the service first using Service Manager to be sure
	m, err := serviceManager()
	if err == nil {
		defer m.Disconnect()
		if s, err := m.OpenService(ws.ServiceName); err == nil {
			defer s.Close()
			s.Control(svc.Stop)
		}
	}

	// Stop the service using WinSW command as well
	stopCmd := exec.Command(ws.WinSWPath, "stop")
	stopCmd.Run() // Ignore errors when stopping, service might already be stopped

	// Uninstall the service
	uninstallCmd := exec.Command(ws.WinSWPath, "uninstall")
...
func (ws *WinSWService) Start() error {
	// Start the service
	startCmd := exec.Command(ws.WinSWPath, "start")
...
func (ws *WinSWService) Restart() error {
	// Restart the service
	restartCmd := exec.Command(ws.WinSWPath, "restart")
