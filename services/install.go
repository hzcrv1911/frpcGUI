...
// Returns profile directory path and the config file name used (frpc.ini or frpc.toml)
func prepareProfileDirectory(configPath string) (string, string, error) {
	...
	// Always use frpc.toml as we dropped INI support
	cfg, err := config.UnmarshalClientConf(configPath)
	if err != nil {
		return "", "", fmt.Errorf("failed to load config: %v", err)
	}
	...
	if err := cfg.SavePure(configDst); err != nil {
		return "", "", fmt.Errorf("failed to save config file: %v", err)
	}

	return profileDir, configFileName, nil
}
...
func StartWinSWService(configPath string) error {
	...
	profileConfigPath, err := getProfileConfigPath(profileDir)
	if err != nil {
		return err
	}
...
// getProfileConfigPath returns the path to the config file in the profile directory
func getProfileConfigPath(profileDir string) (string, error) {
	iniPath := filepath.Join(profileDir, "frpc.ini")
	tomlPath := filepath.Join(profileDir, "frpc.toml")
	...
	return "", fmt.Errorf("config file not found in profile directory")
}
