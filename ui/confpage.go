...
func (cp *ConfPage) OnCreate() {
	...
	cleanup, err := services.WatchConfigServices(func() []string {
		...
		// DEBUG
		if logFile, _ := os.Create(filepath.Join("logs", "state_callback.log")); logFile != nil {
			fmt.Fprintf(logFile, "[DEBUG] State callback: path=%s, state=%d\n", path, state)
			logFile.Close()
		}
		cp.Synchronize(func() {
