...
func (pv *ProxyView) startTracker(refresh bool) bool {
	if pv.tracker == nil && pv.model != nil {
		// In WinSW mode, we need to pass the config path to the tracker
		configPath := getCurrentConf().Path
		pv.tracker = NewProxyTrackerWithConfig(pv.Form(), pv.model, configPath, refresh)
		return true
	}
	return false
}
...
func (pv *ProxyView) onClipboardImport() {
	...
	if strings.HasPrefix(text, "[[proxies]]") {
		// For now, we don't support importing proxy configurations from clipboard
		// when using WinSW, as this would require more complex parsing
		showErrorMessage(pv.Form(), "", i18n.Sprintf("This feature is not supported when using WinSW integration."))
		return
	} else if strings.HasPrefix(text, "[[visitors]]") {
		showErrorMessage(pv.Form(), "", i18n.Sprintf("This feature is not supported when using WinSW integration."))
		return
	} else {
		showErrorMessage(pv.Form(), "", i18n.Sprintf("This feature only supports text in TOML format."))
		return
	}
}
...
func (pv *ProxyView) onEdit(proxy *config.Proxy, create bool) {
	...
	dlg := NewEditProxyDialog(proxy, pv.visitors(except), create, pv.model.HasName)
