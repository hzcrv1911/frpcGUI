...
func (conf *Conf) Delete() error {
	...
	// Delete service config files in the same directory
	configDir := filepath.Dir(conf.Path)
	os.Remove(filepath.Join(configDir, "frpc.ini"))
	os.Remove(filepath.Join(configDir, "frpc.toml"))
...
func (conf *Conf) Save() error {
	...
	// Ensure log directory exists in profile directory
	logDir := filepath.Join(configDir, "logs")
	if err := os.MkdirAll(logDir, os.ModePerm); err != nil {
		return err
	}

	logPath, err := filepath.Abs(filepath.Join(logDir, "frpc.log"))
	if err != nil {
		return err
	}
	...
	// Sync to service config file if exists
	// When the service is installed, there will be a frpc.toml in the same directory
	// We use absolute path to ensure we find the correct file
	absConfigDir, err := filepath.Abs(configDir)
	if err != nil {
		return nil
	}

	tomlPath := filepath.Join(absConfigDir, "frpc.toml")

	// Update frpc.toml if it exists
	if _, err := os.Stat(tomlPath); err == nil {
		conf.Data.SavePure(tomlPath)
	}
...
func loadAllConfs() ([]*Conf, error) {
	...
	for _, f := range files {
		if conf, err := config.UnmarshalClientConf(f); err == nil {
			// Fix relative log path
			if conf.LogFile != "" && !filepath.IsAbs(conf.LogFile) {
				conf.LogFile = filepath.Join(filepath.Dir(f), conf.LogFile)
			}
